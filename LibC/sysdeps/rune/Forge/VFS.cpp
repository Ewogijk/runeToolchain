...
namespace Forge {
    ...
    Ember::StatusCode vfs_get_node_info(const char* node_path, Ember::NodeInfo* node_info_out) {
        return system_call2(Ember::VFS::GET_NODE_INFO,
                            reinterpret_cast<uintptr_t>(node_path),
                            reinterpret_cast<uintptr_t>(node_info_out));
    }

    Ember::StatusCode vfs_get_node_info_by_ID(U16 node_ID, Ember::NodeInfo* node_info_out) {
        return system_call2(Ember::VFS::GET_NODE_INFO_BY_ID,
                            node_ID,
                            reinterpret_cast<uintptr_t>(node_info_out));
    }
    ...
    Ember::StatusCode vfs_directory_stream_next(U16              dir_stream_handle,
                                                Ember::NodeInfo* node_info_ptr) {
        return system_call2(Ember::VFS::DIRECTORY_STREAM_NEXT,
                            dir_stream_handle,
                            (uintptr_t) node_info_ptr);
    }
