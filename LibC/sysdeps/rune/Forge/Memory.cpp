...
namespace Forge {
    ...
    void* memory_allocate_page(void*                       v_addr,
                               const size_t                num_pages,
                               const Ember::PageProtection page_protection) {
        return reinterpret_cast<void*>(system_call3(Ember::Memory::ALLOCATE_PAGE,
    ...
    int memory_free_page(void* v_addr, const size_t num_pages) {
        return system_call2(Ember::Memory::FREE_PAGE,
                            (U64) reinterpret_cast<uintptr_t>(v_addr),
                            num_pages);
    }
} // namespace Forge
