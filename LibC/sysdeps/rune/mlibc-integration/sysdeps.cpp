...
namespace frg FRG_VISIBILITY {
    ...
    class hash<U16> {
      public:
        unsigned int operator()(U16 v) const { return v; }
    };
} // namespace frg FRG_VISIBILITY
...
namespace [[gnu::visibility("hidden")]] mlibc {
    ...
    [[noreturn, gnu::weak]] void sys_thread_exit() { Forge::syscall_not_ported("sys_thread_exit"); }
    ...
    [[gnu::weak]] int sys_prepare_stack(void**  stack,
                                        void*   entry,
                                        void*   user_arg,
                                        void*   tcb,
                                        size_t* stack_size,
    ...
    int sys_futex_wake(int* pointer) { Forge::syscall_not_ported("sys_futex_wake"); }
    ...
    int sys_open(const char* pathname, int flags, mode_t mode, int* fd) {
        ...
        if (flags & O_CREAT) {
            ...
            if (st_code != Ember::Status::OKAY && st_code != Ember::Status::NODE_EXISTS) {
                switch (st_code) {
                    ...
                    case Ember::Status::BAD_ARG: return EINVAL;
        ...
        if (st_code < Ember::Status::OKAY) {
            switch (st_code) {
                case Ember::Status::BAD_ARG:        return EINVAL;
                case Ember::Status::NODE_NOT_FOUND: return ENOENT;
                case Ember::Status::IO_ERROR:       return ENXIO;
                default:                            return ENXIO;
            }
        }
        *fd = st_code;
    ...
    [[gnu::weak]] int sys_flock(int fd, int options) { Forge::syscall_not_ported("sys_flock"); }
    ...
    int sys_write(int fd, const void* buf, size_t count, ssize_t* bytes_written) {
        ...
        switch (fd) {
            ...
            default: {
                ...
                if (ret < 0) {
                    if (ret == Ember::Status::UNKNOWN_ID || ret == Ember::Status::NODE_IS_DIRECTORY
                        || ret == Ember::Status::NODE_CLOSED || ret == Ember::Status::ACCESS_DENIED)
                        return EBADF;
    ...
    int sys_seek(int fd, off_t offset, int whence, off_t* new_offset) {
        ...
        if (whence == SEEK_SET)
            seek_mode = Ember::SeekMode::BEGIN;
        else if (whence == SEEK_CUR)
            seek_mode = Ember::SeekMode::CURSOR;
        else
            seek_mode = Ember::SeekMode::END;
    ...
    [[gnu::weak]] int sys_isatty(int fd) { return fd >= 0 && fd <= 2 ? 0 : 1; }
    [[gnu::weak]] int sys_rmdir(const char* path) { Forge::syscall_not_ported("sys_rmdir"); }
    ...
    [[gnu::weak]] int sys_fork(pid_t* child) { Forge::syscall_not_ported("sys_fork"); }
    [[gnu::weak]] int
    ...
    [[gnu::weak]] pid_t sys_getpid() { return Forge::app_get_ID(); }
    ...
    [[gnu::weak]] int sys_brk(void** out) { Forge::syscall_not_ported("sys_brk"); }

    [[gnu::weak]] int sys_getloadavg(double* samples) {
        Forge::syscall_not_ported("sys_getloadavg");
    }

    [[gnu::weak]] int sys_openpty(int*                  mfd,
                                  int*                  sfd,
                                  char*                 name,
                                  const struct termios* ios,
                                  const struct winsize* win) {
        Forge::syscall_not_ported("sys_openpty");
    }
    ...
    [[gnu::weak]] int sys_iopl(int level) { Forge::syscall_not_ported("sys_iopl"); }
    ...
    [[noreturn]] void sys_libc_panic() { while (true); }

    int sys_tcb_set(void* pointer) { return Forge::threading_set_thread_control_block(pointer); }

    [[gnu::weak]] int sys_futex_tid() { return Forge::threading_get_thread_ID(); }
    ...
    int sys_anon_allocate(size_t size, void** pointer) {
        ...
        void* mem = Forge::memory_allocate_page(nullptr, num_pages, Ember::PageProtection::WRITE);
    ...
    // mlibc assumes that anonymous memory returned by sys_vm_map() is zeroed by the kernel /
    // whatever is behind the sysdeps
    int
    sys_vm_map(void* hint, size_t size, int prot, int flags, int fd, off_t offset, void** window) {
        Forge::syscall_not_ported("sys_vm_map");
    }
    int sys_vm_unmap(void* pointer, size_t size) { Forge::syscall_not_ported("sys_vm_unmap"); }
    ...
    //    }
    //
    //    [[gnu::weak]] int sys_statx(int dirfd, const char *pathname, int flags, unsigned int mask,
    ...
    //    }
    //
    //    [[gnu::weak]] int sys_sendfile(int outfd, int infd, off_t *offset, size_t count, ssize_t
    //    *out) {
    ...
    //    }
    //    [[gnu::weak]] int sys_pidfd_send_signal(int pidfd, int sig, siginfo_t *info, unsigned int
    //    flags) {
    ...
    [[gnu::weak]] int
    sys_writev(int fd, const struct iovec* iovs, int iovc, ssize_t* bytes_written) {
        Forge::syscall_not_ported("sys_writev");
    }

    [[gnu::weak]] int
    sys_pwrite(int fd, const void* buf, size_t n, off_t off, ssize_t* bytes_read) {
    ...
    [[gnu::weak]] ssize_t sys_sendto(int                    fd,
                                     const void*            buffer,
                                     size_t                 size,
                                     int                    flags,
    ...
    [[gnu::weak]] ssize_t sys_recvfrom(int              fd,
                                       void*            buffer,
                                       size_t           size,
                                       int              flags,
    ...
    [[gnu::weak]] int   sys_listen(int fd, int backlog) { Forge::syscall_not_ported("sys_listen"); }
    [[gnu::weak]] gid_t sys_getgid() { Forge::syscall_not_ported("sys_getgid"); }
    [[gnu::weak]] gid_t sys_getegid() { Forge::syscall_not_ported("sys_getegid"); }
    [[gnu::weak]] uid_t sys_getuid() { Forge::syscall_not_ported("sys_getuid"); }
    [[gnu::weak]] uid_t sys_geteuid() { Forge::syscall_not_ported("sys_geteuid"); }
    [[gnu::weak]] pid_t sys_gettid() { Forge::syscall_not_ported("sys_gettid"); }
    [[gnu::weak]] pid_t sys_getppid() { Forge::syscall_not_ported("sys_getppid"); }
    [[gnu::weak]] int   sys_getpgid(pid_t pid, pid_t* pgid) {
        Forge::syscall_not_ported("sys_getpgid");
    }
    [[gnu::weak]] int sys_getsid(pid_t pid, pid_t* sid) { Forge::syscall_not_ported("sys_getsid"); }
    [[gnu::weak]] int sys_setpgid(pid_t pid, pid_t pgid) {
        Forge::syscall_not_ported("sys_setpgid");
    }
    [[gnu::weak]] int sys_setuid(uid_t uid) { Forge::syscall_not_ported("sys_setuid"); }
    [[gnu::weak]] int sys_seteuid(uid_t euid) { Forge::syscall_not_ported("sys_seteuid"); }
    [[gnu::weak]] int sys_setgid(gid_t gid) { Forge::syscall_not_ported("sys_setgid"); }
    [[gnu::weak]] int sys_setegid(gid_t egid) { Forge::syscall_not_ported("sys_setegid"); }
    [[gnu::weak]] int sys_getgroups(size_t size, gid_t* list, int* ret) {
        Forge::syscall_not_ported("sys_getgroups");
    }
    [[gnu::weak]] void sys_yield() { Forge::syscall_not_ported("sys_yield"); }
    [[gnu::weak]] int  sys_pselect(int                    num_fds,
                                   fd_set*                read_set,
                                   fd_set*                write_set,
                                   fd_set*                except_set,
    ...
    [[gnu::weak]] int sys_chdir(const char* path) { Forge::syscall_not_ported("sys_chdir"); }
    [[gnu::weak]] int sys_fchdir(int fd) { Forge::syscall_not_ported("sys_fchdir"); }
    [[gnu::weak]] int sys_chroot(const char* path) { Forge::syscall_not_ported("sys_chroot"); }
    ...
    [[gnu::weak]] void sys_sync() { Forge::syscall_not_ported("sys_sync"); }
    [[gnu::weak]] int  sys_fsync(int fd) { Forge::syscall_not_ported("sys_fsync"); }
    [[gnu::weak]] int  sys_fdatasync(int fd) { Forge::syscall_not_ported("sys_fdatasync"); }
    [[gnu::weak]] int  sys_chmod(const char* pathname, mode_t mode) {
        Forge::syscall_not_ported("sys_chmod");
    }
    [[gnu::weak]] int sys_fchmod(int fd, mode_t mode) { Forge::syscall_not_ported("sys_fchmod"); }
    ...
    [[gnu::weak]] int sys_mlockall(int flags) { Forge::syscall_not_ported("sys_mlockall"); }
    [[gnu::weak]] int sys_munlockall(void) { Forge::syscall_not_ported("sys_munlockall"); }
    ...
    [[gnu::weak]] int sys_setsid(pid_t* sid) { Forge::syscall_not_ported("sys_setsid"); }
    ...
    [[gnu::weak]] int sys_tcflow(int, int) { Forge::syscall_not_ported("sys_tcflow"); }
    [[gnu::weak]] int sys_tcflush(int fd, int queue) { Forge::syscall_not_ported("sys_tcflush"); }
    [[gnu::weak]] int sys_tcdrain(int) { Forge::syscall_not_ported("sys_tcdrain"); }
    [[gnu::weak]] int sys_pipe(int* fds, int flags) { Forge::syscall_not_ported("sys_pipe"); }
    ...
    [[gnu::weak]] int sys_getsockopt(int fd,
                                     int layer,
                                     int number,
    ...
    [[gnu::weak]] int
    sys_setsockopt(int fd, int layer, int number, const void* buffer, socklen_t size) {
    ...
    [[gnu::weak]] int sys_sigtimedwait(const sigset_t* __restrict set,
    ...
    [[gnu::weak]] int sys_sigpending(sigset_t* set) { Forge::syscall_not_ported("sys_sigpending"); }
    ...
    [[gnu::weak]] int sys_timer_settime(timer_t t,
                                        int     flags,
                                        const struct itimerspec* __restrict val,
                                        struct itimerspec* __restrict old) {
        Forge::syscall_not_ported("sys_timer_settime");
    }
    [[gnu::weak]] int sys_timer_delete(timer_t t) { Forge::syscall_not_ported("sys_timer_delete"); }
    [[gnu::weak]] int sys_times(struct tms* tms, clock_t* out) {
        Forge::syscall_not_ported("sys_times");
    }
    [[gnu::weak]] int sys_uname(struct utsname* buf) { Forge::syscall_not_ported("sys_uname"); }
    [[gnu::weak]] int sys_pause() { Forge::syscall_not_ported("sys_pause"); }
    ...
    [[gnu::weak]] int sys_unlockpt(int fd) { Forge::syscall_not_ported("sys_unlockpt"); }
    ...
    [[gnu::weak]] int sys_sysconf(int num, long* ret) { Forge::syscall_not_ported("sys_sysconf"); }
    ...
    [[gnu::weak]] int sys_name_to_handle_at(int                 dirfd,
                                            const char*         pathname,
    ...
    [[gnu::weak]] int sys_splice(int          in_fd,
                                 off_t*       in_off,
                                 int          out_fd,
                                 off_t*       out_off,
                                 size_t       size,
    ...
    [[gnu::weak]] int sys_shmdt(const void* shmaddr) { Forge::syscall_not_ported("sys_shmdt"); }
